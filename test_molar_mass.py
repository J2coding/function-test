...
from molar_mass import make_periodic_table2, \
    parse_formula, compute_molar_mass
...
def test_make_periodic_table2():
    """Verify that the make_periodic_table2 function works correctly.
    Parameters: none
    Return: nothing
    """
    # Call the make_periodic_table2 function and store the returned
    # dictionary in a variable named periodic_table_dict.
    periodic_table_dict = make_periodic_table2()

    # Verify that the make_periodic_table2 function returns a dictionary.
    assert isinstance(periodic_table_dict, dict), \
        "make_periodic_table2 function must return a dictionary: " \
...
def test_parse_formula():
    ...
    assert parse_formula("H2O", periodic_table_dict) \
        == [('H', 2), ('O', 1)]
    assert parse_formula("PO4H2(CH2)12CH3", periodic_table_dict) \
        == [("P", 1), ("O", 4), ("H", 29), ("C", 13)]

    assert parse_formula("Fe2O3", periodic_table_dict) \
        == [("Fe", 2), ("O", 3)]
...
def test_compute_molar_mass():
    ...
    assert compute_molar_mass([('H', 2), ('O', 1)], periodic_table_dict) \
        == approx(18.01528)
